...
extern void mod1__ode_solver(int *neq,
			   double *theta,      //order:
			   double *time,
			   int *evid,
			   int *ntime,
  ...
  //if (_ptrid() != 1541617163 ){ _assign_ptr(mod1__model_vars());}
  double *_theta = theta;
...
void mod1__dydt(int *_neq, double t, double *__zzStateVar__, double *__DDtStateVar__)
{
  ...
  __DDtStateVar__[1]=_IR[1]+KA*depot-(CL+Q+Q2)*Concentration+Q*C3+Q2*C4;
  __DDtStateVar__[2]=_IR[2]+Q*Concentration-Q*C3;
  __DDtStateVar__[3]=_IR[3]+Q2*Concentration-Q2*C4;
  __DDtStateVar__[4]=_IR[4]+Kin*(1+Emax*Concentration/safe_zero((EC50+Concentration)))-Kout*(1)*eff;
...
void mod1__calc_lhs(int _cSub, double t, double *__zzStateVar__, double *_lhs) {
  ...
  __DDtStateVar_1__=_IR[1]+KA*depot-(CL+Q+Q2)*Concentration+Q*C3+Q2*C4;
  __DDtStateVar_2__=_IR[2]+Q*Concentration-Q*C3;
  __DDtStateVar_3__=_IR[3]+Q2*Concentration-Q2*C4;
  __DDtStateVar_4__=_IR[4]+Kin*(1+Emax*Concentration/safe_zero((EC50+Concentration)))-Kout*(1)*eff;
...
extern SEXP mod1__model_vars(){
  ...
  if (!_rxIsCurrentC(_mv)){
    ...
    SEXP initsr   = PROTECT(allocVector(REALSXP, 5));pro++;
    SEXP scaler   = PROTECT(allocVector(REALSXP, 5));pro++;
    SEXP infusionr= PROTECT(allocVector(REALSXP, 5));pro++;
    SEXP badDosei = PROTECT(allocVector(INTSXP, 5));pro++;
  ...
  SET_STRING_ELT(lhs,2,mkChar("C4"));
    ...
    SET_STRING_ELT(mmd5,0,mkChar("92cb49a278e6acf5b71327ee5ea5cf3f"));
    SET_STRING_ELT(mmd5n,1,mkChar("parsed_md5"));
    SET_STRING_ELT(mmd5,1,mkChar("1a4b21b916cff4c096b33f03497aaac1"));
    ...
    SET_STRING_ELT(tran, 7,mkChar("mod1__ode_solver"));
