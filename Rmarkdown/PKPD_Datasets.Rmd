...
This page describes the master PKPD analysis datasets used in the majority of the plots on this site. The datasets were generated using RxODE. Data specifications can be accessed on [Datasets](Datasets.html). One dataset was created for single ascending dose PK and another dataset for multiple ascending dose PK and PD. Explore the datasets in the tables below, or download from the following links:

* [simulated dataset of single ascending dose PK](Data/Single_Ascending_Dose_Dataset2.csv)
* [master PKPD dataset for multiple ascending dose PK and PD](Data/Multiple_Ascending_Dose_Dataset2.csv)

The generation code below is shown for reference but is not run when the site is
built. The datasets it produces are committed under `Data/`, and the tables on
this page are rendered from those files. See "Regenerating the datasets" at the
foot of the page.

```{r setup, include = FALSE}
# Dataset generation is opt-in, and off by default, for two reasons. It needs
# rxode2, which requires C and Fortran compilers and is not needed to build any
# other page. And it writes to Data/*.csv -- the committed datasets that every
# other page reads -- so running it during a build would let the site quietly
# regenerate its own inputs.
regenerate <- Sys.getenv("XGX_REGENERATE_DATA") == "true"
```

## Setup

```{r, eval = regenerate, error = TRUE, echo = TRUE, warning=FALSE, message=FALSE}
library(dplyr)
library(rxode2)
```

```{r, include = FALSE}
# When not regenerating, load the committed datasets so the tables below render.
if (!regenerate) {
  Single_Ascending_Dose_Dataset2 <-
    read.csv("../Data/Single_Ascending_Dose_Dataset2.csv")
  Multiple_Ascending_Dose_Dataset2 <-
    read.csv("../Data/Multiple_Ascending_Dose_Dataset2.csv")
}
```
...
```{r, eval = regenerate, error = TRUE, cache = TRUE, echo = TRUE}
ode <- "
...
```{r, error = TRUE, echo = TRUE, warning = FALSE}
...
```{r, eval = regenerate, error = TRUE, cache = TRUE, echo = TRUE, warning = FALSE}
set.seed(12345666)
ndose <- 6
nsub <- 10 # number of subproblems
...
The code on this page is not evaluated during a normal site build, for two
reasons. It needs `rxode2`, which requires C and Fortran compilers and is not
needed by any other page. And it writes to `Data/*.csv` — the committed
datasets that every other page on this site reads — so running it as part of a
build would let the site quietly regenerate its own inputs.

The datasets are therefore treated as source: generated deliberately, checked,
and committed. To regenerate them:
...
Review the diff to `Data/` before committing: the code is seeded, but the
results still depend on the R version and on the ODE solver, so the numbers can
move. This page previously used `RxODE`, which was archived from CRAN on
2022-10-10; the migration to `rxode2` has not been run end to end, so expect to
verify the output the first time.
