...
# Dataset generation depends on RxODE, which was archived from CRAN on
# 2022-10-10. Evaluating it during a normal site build would make the whole
# site unbuildable on any machine without an old RxODE already installed, and
# would also silently overwrite the committed datasets that every other page
# reads. So generation is opt-in, and off by default.
...
reasons. It depends on `RxODE`, which was archived from CRAN on 2022-10-10 and
can no longer be installed from there. And it writes to `Data/*.csv` — the
committed datasets that every other page on this site reads — so running it as
part of a build would let the site quietly regenerate its own inputs.

The datasets are therefore treated as source: generated deliberately, checked,
and committed. To regenerate them you need `RxODE` installed, and then:

```bash
XGX_REGENERATE_DATA=true Rscript -e 'rmarkdown::render("PKPD_Datasets.Rmd")'
```

Run from the `Rmarkdown/` directory. Review the diff to `Data/` before
committing: the code is seeded, but the results still depend on the R version
and on RxODE's solver, so the numbers can move.
