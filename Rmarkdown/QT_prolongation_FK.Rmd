...
author: "Chase Brown, Fariba Khanshan, Alison Margolskee, Andy Stein"
output:
  html_document:
    toc: true
    toc_float: true
...
source(paste0(xgxr_master_url, "R/predict.nls.R"))
...
# Load Dataset
qt_data <- read.csv("../Data/QT_Prolongation_dataset.csv")
...
Drug-induced arrhythmias, such as Torsades de Pointes, are life-threatening adverse events and are therefore important when considering certain aspects of a drug's toxicity. These drug-induced arrhythmias can be studied with QT interval data from an ECG.
...
Changes to this QT interval after drug treatment, which we denote here as $\Delta QT$ (and the heart rate corrected version $\Delta QTc_F$
...
<!--START_EXPLANATION-->
Summarize the $\Delta QTc_F$ data for each dose with a simple table showing the mean and confidence intervals information for the maximum $\Delta QTc_F$ for each individual.
<!--END_EXPLANATION-->
...
# PK Data
gg <- ggplot(data = qt_data %>% filter(PROFDAY %in% c(1,2) | SS), aes(x = PROFTIME,y = LIDV,color = DOSE_FACTOR,fill = DOSE_FACTOR))
gg <- gg + xgx_stat_ci(conf_level = .95)
gg <- gg + xgx_annotate_status(status)
gg <- gg + xgx_scale_x_time_units(units_dataset = time_units_dataset, 
                                  units_plot    = time_units_dataset)
...
```{r, error = TRUE , cache = TRUE, echo = TRUE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 3.5}

# Same QT vs Concentration for each plot
gg <- ggplot(data = qt_data,aes(x = LIDV,y = DELTA_QT))
gg <- gg + geom_point(alpha = 0.25)
...
Note that `geom_smooth` can only recognize `y` and `x` variables in formulas. If you want to include additional variables, e.g. covariates or subject ID (e.g. for repeated measures), this requires the model to be fit outside of geom_smooth. Below we include two examples of mixed effects models using `lmer` from the `lme4` package, and `predictInterval` from the `merTools` package.

```{r, error = TRUE , cache = TRUE, echo = TRUE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 3.5}
model <- lme4::lmer(formula = DELTA_QT ~ 1 + LIDV + (1 | ID), data = qt_data)

gg3 <- gg + geom_smooth(mapping = aes(y = data.frame(merTools::predictInterval(model))$fit),se = FALSE)
gg3 <- gg3 + geom_smooth(mapping = aes(y = data.frame(merTools::predictInterval(model))$lwr),se = FALSE, linetype = "dotted")
gg3 <- gg3 + geom_smooth(mapping = aes(y = data.frame(merTools::predictInterval(model))$upr),se = FALSE, linetype = "dotted")
title = "Mixed Effects"
subtitle <- latex2exp::TeX("$\\Delta QTc_{F, ij} = (\\beta_0 + b_{0i}) + \\beta_1 * Conc_{ij} + \\epsilon_{ij}$")
gg3 <- gg3 + ggtitle(label = title, subtitle = subtitle)
gg3 <- gg3+ theme(legend.position="none")


model <- lme4::lmer(formula = DELTA_QT ~ 1 + logLIDV + (1 | ID), data = qt_data %>% mutate(logLIDV = log(LIDV + 1)))

gg4 <- gg + geom_smooth(mapping = aes(y = data.frame(merTools::predictInterval(model))$fit),se = FALSE)
gg4 <- gg4 + geom_smooth(mapping = aes(y = data.frame(merTools::predictInterval(model))$lwr),se = FALSE, linetype = "dotted")
gg4 <- gg4 + geom_smooth(mapping = aes(y = data.frame(merTools::predictInterval(model))$upr),se = FALSE, linetype = "dotted")
title <- "Mixed Effects (log)"
subtitle <- latex2exp::TeX("$\\Delta QTc_{F, ij} = (\\beta_0 + b_{0i}) + log(1 + \\beta_1 * Conc_{ij}) + \\epsilon_{ij}$")
gg4 <- gg4 + ggtitle(label = title, subtitle = subtitle)
...
<!--START_EXPLANATION-->
In addition to observing the relationship between $\Delta QTc_F$ and concentration 
...
```{r, error = TRUE , cache = TRUE, echo = TRUE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 5}
model <- lme4::lmer(formula = DELTA_QT_max ~ 1 + VALUE + (1 | ID), data = NCA)

gg <- ggplot(data = NCA,aes(x = VALUE, y = DELTA_QT_max))
gg <- gg + geom_smooth(method = "lm", mapping = aes(y = data.frame(merTools::predictInterval(model))$fit), se = FALSE)
gg <- gg + geom_smooth(method = "lm", mapping = aes(y = data.frame(merTools::predictInterval(model))$lwr), se = FALSE, linetype = "dotted")
gg <- gg + geom_smooth(method = "lm", mapping = aes(y = data.frame(merTools::predictInterval(model))$upr), se = FALSE, linetype = "dotted")
gg <- gg + geom_point()
gg <- gg + xgx_annotate_status(status)
...
```{r, error = TRUE , cache = TRUE, echo = TRUE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 4}
gg <- ggplot(data = qt_data,aes(x = LIDV,y = DELTA_QT, color = SEX, fill = SEX))
gg <- gg + xgx_geom_smooth_emax()
gg <- gg + geom_point(alpha = 0.25, aes(shape = WEIGHT>70)) 
gg <- gg + xgx_stat_ci(conf_level = .95, bins = 5, geom = c("errorbar", "point"))
gg <- gg + xgx_annotate_status(status)
gg <- gg + guides(color = guide_legend(""), fill = guide_legend(""))
gg <- gg + labs(x = conc_label, y = qt_label)
...
```{r, error = TRUE , cache = TRUE, echo = TRUE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 5}
model <- lme4::lmer(formula = DELTA_QT_max ~ 1 + VALUE + (1 | ID) + SEX, data = NCA)

gg <- ggplot(data = NCA,aes(x = VALUE,y = DELTA_QT_max,fill = SEX,color = SEX))
gg <- gg + geom_smooth(method = "lm", mapping = aes(y = data.frame(merTools::predictInterval(model))$fit), se = FALSE)
gg <- gg + geom_smooth(method = "lm", mapping = aes(y = data.frame(merTools::predictInterval(model))$lwr), se = FALSE, linetype = "dotted")
gg <- gg + geom_smooth(method = "lm", mapping = aes(y = data.frame(merTools::predictInterval(model))$upr), se = FALSE, linetype = "dotted")
...
gg <- ggplot(data = qt_data, aes(x = LIDV,y = DELTA_QT,color = TIME))
