...
```{r, error = TRUE , cache = TRUE, echo = TRUE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 7}
...
qt_conc_plot <- function(model_num){
  ...
  if(model_num == 1){
    ...
    model <- lm(formula = DELTA_QT ~ 1 + LIDV + ID, data = qt_data)

    gg <- gg + geom_smooth(mapping = aes(y = data.frame(predict(model, interval = "confidence"))$fit),se = FALSE)
    gg <- gg + geom_smooth(mapping = aes(y = data.frame(predict(model, interval = "confidence"))$lwr),se = FALSE, color = "gray")
    gg <- gg + geom_smooth(mapping = aes(y = data.frame(predict(model, interval = "confidence"))$upr),se = FALSE,color = "gray")
    
    title = "Mixed Effects"
    subtitle <- latex2exp::TeX("$\\Delta QTc_{F, ij} = (\\beta_0 + b_{0i}) + \\beta_1 * Conc_{ij} + \\epsilon_{ij}$")
  }
  else if(model_num == 4){
    model <- lm(formula = DELTA_QT ~ 1 + log(1 + LIDV) + ID, data = qt_data)
...
```{r, error = TRUE , cache = TRUE, echo = TRUE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 5}
model <- lm(formula = DELTA_QT_max ~ 1 + VALUE + ID,data = NCA)
