...
When we model the relationship between exposure and response, we almost always implicitly assume that our models can be interpreted in a causal framework, such that changing exposure (by changing the dose regimen) will change the response. However, there can be factors that correlate with both exposure and response that confound this interpretation (see [Simpson’s Paradox](https://en.wikipedia.org/wiki/Simpson%27s_paradox) and it’s important we check for these factors as best we can.  For example, for many PD-1 and PD-L1 inhibitors, if one only looks at steady state exposure at a single dose, one may detect an exposure-response trend -- however the trend is due to the fact that responders exhibit a reduction in clearance, and therefore have greater steady-state exposure [1].  Interpreting the exposure-response analysis as if the steady-state exposure caused a greater response rate can lead to erroneous conclusions and poor decisions.
...
* Identify any forks or feedback loops in the causal graph
  * For any forks, stratify exposure-response plots by the variable.  If the variable is continuous, you may need to introduce thresholds.
