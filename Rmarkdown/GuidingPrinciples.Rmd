...
When we model the relationship between exposure and response, we almost always implicitly assume that our models can be interpreted in a causal framework, such that changing exposure (by changing the dose regimen) will change the response. However, there can be factors that correlate with both exposure and response that can confound this interpretation (c.f. (Simpson’s Paradox)[https://en.wikipedia.org/wiki/Simpson%27s_paradox]) and it’s important we check for these factors as best we can.  For example, for many PD-1 and PD-L1 inhibitors, if one only looks at steady state exposure at a single dose, one may detect an exposure-response trend -- however the trend is due to the fact that responders exhibit a reduction in clearance, and therefore have greater exposure (Liu et al., 2017).  Interpreting the exposure-response analysis as if the exposure caused a greater response rate can lead to erroneous conclusions and poor decisions.

When performing an exploratory Exposure-Response analysis that takes causality into account, you should
