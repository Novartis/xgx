...
gg <- ggplot(data = pk_data, 
             aes(x = NOMTIME, y = LIDV, 
                 group = interaction(DOSE, CYCLE), color = TRTACT_high2low)) 
gg <- gg + xgx_theme()
...
gg <- ggplot(data = pk_data, aes(x = NOMTIME, y = LIDV, group = interaction(DOSE, CYCLE)))
gg <- gg + xgx_theme()
gg <- gg + geom_line(aes(group = interaction(ID, CYCLE)), color = rgb(0.5, 0.5, 0.5), size = 1, alpha = 0.3)  
...
gg <- gg + xgx_theme()
gg <- gg + geom_line(aes(group = interaction(ID, CYCLE), color = TRTACT_high2low), size = 1, alpha = 0.5) 
...
gg <- gg + geom_line(data = pk_data_rep_by_trt, 
                     aes(group = interaction(ID_rep_by_trt, CYCLE)), 
...
gg <- gg + geom_line(data = pk_data, 
                     aes(group = interaction(ID, CYCLE)), size = 1)
