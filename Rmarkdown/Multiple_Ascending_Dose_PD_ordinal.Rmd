...
pkpd_data[pkpd_data$CMT!=5,]$Severity <- NA
pkpd_data$Severity_label <- factor(plyr::mapvalues(pkpd_data$Severity,c(1,2,3), c("Mild", "Moderate","Severe")), levels = c("Mild","Moderate","Severe"))

#create a factor for the treatment variable for plotting
pkpd_data$DAY <- floor(pkpd_data$TIME/24 )
...
pd_data_d5 <- pd_data %>% subset(DAY_label%in%c("Day 5"),)
...
pd_data_d135 <- pd_data %>% subset(DAY_label%in%c("Day 1","Day 3","Day 5"),)
