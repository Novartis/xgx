...
data_to_plot <- list()
data_to_plot[[1]] <- pk_data
data_to_plot[[2]] <- pd_data

ggs <- list()
for(iplot in 1:length(data_to_plot)){
  data_to_plot[[iplot]]$TRTACT_low2high <- factor(data_to_plot[[iplot]]$TRTACT_low2high, 
                                       levels = rev(levels(data_to_plot[[iplot]]$TRTACT_low2high)))
  gg <- ggplot(data = data_to_plot[[iplot]], 
               aes(x=NOMTIME,y=LIDV, color = TRTACT_low2high, fill = TRTACT_low2high))+theme_bw() 
...
data_to_plot <- list()
data_to_plot[[1]] <- pk_data
data_to_plot[[2]] <- pd_data[pd_data$DOSE>0,]

ggs <- list()
for(iplot in 1:length(data_to_plot)){
  gg <- ggplot(data = data_to_plot[[iplot]], 
               aes(x=NOMTIME,y=LIDV))+theme_bw()
  gg <- gg + xgx_stat_ci(conf_level = .95)
  gg <- gg + xgx_annotate_status(status)
  gg <- gg + xgx_scale_x_time_units(units_dataset = time_units_dataset, 
                                  units_plot    = time_units_plot)
...
for(iplot in 1:length(data_to_plot)){
  data_to_plot[[iplot]]$TRTACT_low2high <- factor(data_to_plot[[iplot]]$TRTACT_low2high, 
                                       levels = rev(levels(data_to_plot[[iplot]]$TRTACT_low2high)))
  gg <- ggplot(data = data_to_plot[[iplot]], 
               aes(x=TIME, y=LIDV,group=ID, color =factor(TRTACT_low2high))) + theme_bw()
  gg <- gg + xgx_annotate_status(status)
  ...
  gg <- gg + xgx_scale_x_time_units(units_dataset = time_units_dataset, 
                                  units_plot    = time_units_plot)
  if(iplot == 1){
    gg <- gg + geom_point(data = data_to_plot[[iplot]][data_to_plot[[iplot]]$CENS==1,], color="red", shape=8,alpha = 0.5)
    gg <- gg + xgx_scale_y_log10() 
    gg <- gg + labs(y=pkconc_label)
  }else{
    gg <- gg + labs(y=pdconc_label)
...
ggs <- list()
for(iplot in 1:length(data_to_plot)){
  gg <- ggplot(data = data_to_plot[[iplot]], aes(x=TIME,y=LIDV,group=ID))+theme_bw()
  gg <- gg + xgx_annotate_status(status)
  gg <- gg + geom_line(alpha = 0.5)
  gg <- gg + geom_point(alpha = 0.5)
  gg <- gg + guides(color=guide_legend(""),fill=guide_legend(""))
...
gg <- ggplot(data = data_to_plot2, aes(x=Concentration,y=Response, color = TRTACT_low2high))+theme_bw()
gg <- gg + xgx_annotate_status(status)
gg <- gg + geom_point()
gg <- gg + geom_point(data=data_to_plot2[data_to_plot2$CENS==1,], color="red", shape = 8)
gg <- gg + guides(color=guide_legend(""),fill=guide_legend(""))
...
```{r, cache = TRUE, fig.width = 6, fig.height = 4, warning = FALSE, message= FALSE}
AUC_0_24 <- pk_data[pk_data$NOMTIME>0&pk_data$NOMTIME<=24,]
...
AUCtau <- pk_data[pk_data$NOMTIME>120&pk_data$NOMTIME<=144,]
...
RESPONSE <- pd_data[pd_data$PROFDAY%in%c(1,6),
                    c("ID","PROFDAY","LIDV","DOSE","TRTACT_low2high", "SEX")]
names(RESPONSE) <- c("ID","PROFDAY","Response","DOSE","TRTACT_low2high", "SEX")
...
data_to_plot$TRTACT <- factor(data_to_plot$TRTACT_low2high, 
                                   levels = rev(levels(data_to_plot$TRTACT_low2high)))

gg <- ggplot(data = data_to_plot, aes(x = AUC, y = Response))
gg <- gg + xgx_annotate_status(status)
gg <- gg + geom_point(aes(color = TRTACT)) + geom_smooth(color="black")
gg <- gg + xgx_stat_ci(geom = "errorbar")
gg <- gg + guides(color=guide_legend(""),fill=guide_legend(""))
