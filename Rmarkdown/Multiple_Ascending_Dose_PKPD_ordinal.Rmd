...
gg <- gg + geom_jitter(data = pkpd_data_wide[pkpd_data_wide$CENS==0,],
                       aes(color = Severity_label, shape = Severity_label),  width = 0.1, height = 0.0, alpha = 0.75)
gg <- gg + geom_jitter(data = pkpd_data_wide[pkpd_data_wide$CENS==1,],
...
pkpd_data_wide_d5 <- pkpd_data_wide[pkpd_data_wide$PROFDAY==5,]
gg <- ggplot(data = pkpd_data_wide_d5, aes(y=CONC,x=Severity_label))
gg <- ggplot(data = pkpd_data_wide_d5, aes(y=CONC,x=Severity_label))
gg <- gg + geom_jitter(data = pkpd_data_wide_d5[pkpd_data_wide_d5$CENS==0,],
                       aes(color = TRTACT_high2low), shape=19,  width = 0.1, height = 0.0, alpha = 0.5)
gg <- gg + geom_jitter(data = pkpd_data_wide_d5[pkpd_data_wide_d5$CENS==1,],
