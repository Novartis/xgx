...
#AUC dataset for AUC_0_24 and AUCtau
AUC_0_24 <- NCA[NCA$PARAM=="AUC_0_24",]
AUC_0_24 <- AUC_0_24[c("VALUE","ID")]
names(AUC_0_24) <- c("AUC","ID")
AUC_0_24$PROFDAY = 1

AUCtau <- NCA[NCA$PARAM=="AUC_tau",]
AUCtau <- AUCtau[c("VALUE","ID")]
...
gg <- ggplot(data = pk_data, 
               aes(x=NOMTIME,y=LIDV, color = TRTACT_high2low, fill = TRTACT_high2low))+theme_bw() 
...
#PK data
 gg <- ggplot(data = pk_data, 
               aes(x=TIME, y=LIDV,group=ID, color =factor(TRTACT_high2low))) + theme_bw()
 gg <- gg + xgx_annotate_status(status)
...
```{r, cache = TRUE, fig.width = 10, fig.height = 6, warning = FALSE, message= FALSE}
#PK data
gg <- ggplot(data = pk_data, aes(x=TIME,y=LIDV,group=ID))+theme_bw()
gg <- gg + xgx_annotate_status(status)
gg <- gg + geom_line(alpha = 0.5)
gg <- gg + geom_point(alpha = 0.5)
gg <- gg + guides(color=guide_legend(""),fill=guide_legend(""))
...
#PD data
gg <- ggplot(data = pd_data[pd_data$DOSE>0,], 
             aes(x=TIME,y=LIDV,group=ID))+theme_bw()
gg <- gg + xgx_annotate_status(status)
gg <- gg + geom_line(alpha = 0.5)
gg <- gg + geom_point(alpha = 0.5)
gg <- gg + guides(color=guide_legend(""),fill=guide_legend(""))
