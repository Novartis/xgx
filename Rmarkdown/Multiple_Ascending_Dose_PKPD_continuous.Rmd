...
### PD marker by Concentration, for endpoint of interest, mean (95% CI) by concentration bins
<!--START_EXPLANATION-->
...
### PD marker by Concentration, faceted by visit, mean (95% CI) by concentration bins
...
```{r, error = TRUE, cache = TRUE, fig.width = 8, fig.height = 6, warning = FALSE, message= FALSE}
gg <- ggplot(data = NCA, aes(x = VALUE, y = PD))
gg <- gg + xgx_annotate_status(status)
gg <- gg + geom_point(aes(color = TRTACT_high2low)) 
