...
gg <- ggplot(data = pkpd_data_wide[pkpd_data_wide$PROFDAY== 5,], aes(x=CONC,y=RESPONSE))
gg <- gg + xgx_annotate_status(status)
gg <- gg + geom_point(aes(color = TRTACT_low2high))
gg <- gg + geom_point(data=pkpd_data_wide[pkpd_data_wide$CENS==1,], color="red", shape = 8)
...
pkpd_data_wide$DAY_label <- paste("Day", pkpd_data_wide$PROFDAY)
pkpd_data_wide_day13579 <- pkpd_data_wide[pkpd_data_wide$PROFDAY%in%c(1,3,5,7,9),]

gg <- ggplot(data = pkpd_data_wide_day13579,
             aes(x=CONC,y=RESPONSE, color = TRTACT_low2high))
gg <- gg + xgx_annotate_status(status)
gg <- gg + geom_point()
gg <- gg + geom_point(data=pkpd_data_wide_day13579[pkpd_data_wide_day13579$CENS==1,], color="red", shape = 8)
gg <- gg + guides(color=guide_legend(""),fill=guide_legend(""))
gg <- gg + labs(x=conc_label , y=pd_label)
gg <- gg + xgx_scale_x_log10() 
...
gg <- ggplot(data = pkpd_data_wide, aes(x=CONC,y=RESPONSE, color=TIME))
gg <- gg + xgx_annotate_status(status)
