...
gg <- ggplot(data = pkpd_data_wide %>% subset(PROFDAY == 5,), aes(x = CONC,y = RESPONSE))
gg <- gg + xgx_annotate_status(status)
...
pkpd_data_wide_day13579 <- pkpd_data_wide %>% subset(PROFDAY %in% c(1,3,5,7,9),)

gg <- ggplot(data = pkpd_data_wide_day13579,
             aes(x = CONC, y = RESPONSE, color = TRTACT_high2low))
gg <- gg + xgx_annotate_status(status)
gg <- gg + geom_point()
gg <- gg + geom_point(data = pkpd_data_wide_day13579 %>% subset(CENS==1,), color="red", shape = 8)
gg <- gg + guides(color = guide_legend(""), fill = guide_legend(""))
gg <- gg + labs(x = conc_label , y = pd_label)
gg <- gg + xgx_scale_x_log10() 
...
pkpd_data_wide <- pkpd_data_wide %>% arrange(TIME)
