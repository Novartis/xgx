...
NCA <- pd_data[pd_data$PROFDAY%in%c(1,6),] %>%
                       select(ID,PROFDAY,Response = LIDV,TRTACT_low2high,TRTACT_high2low) %>%
...
```{r, error = TRUE, cache = TRUE, fig.width = 6, fig.height = 4, warning = FALSE, message= FALSE}
...
gg <- ggplot(data = pkpd_data_wide_day13579,
             aes(x=CONC,y=RESPONSE, color = TRTACT_high2low))
gg <- gg + xgx_annotate_status(status)
gg <- gg + geom_point()
