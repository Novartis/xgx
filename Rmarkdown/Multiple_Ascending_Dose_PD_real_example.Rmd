...
data_to_plot <- pd_data
data_to_plot_single_dosing <- data_to_plot %>% subset(MAD == 0)
...
ytitle <- ylab(paste0(pd_label,"\nchange from baseline"))
xtitle <- xlab("Time (days)")

plot2 <- ggplot(data_to_plot_multiple_dosing, 
...
plot1 <- ggplot(data_to_plot_single_dosing, 
                aes(y = CHG, x = factor(DOSE), group = NOMTIME)) + 
  ...
  theme(axis.text = element_text(size = 13),
        axis.title = element_text(size = 14),
        strip.text = element_text(size = 14)) +
  facet_wrap(~NOMTIME, nrow = 1, label = labelFun)
...
plot1b <- ggplot(data_to_plot_single_dosing %>% subset(NOMTIME == 24,), 
               aes(y = CHG, x = DOSE, group = NOMTIME)) + 
  xgx_stat_ci(conf_level = 0.975, geom = c("point", "errorbar"), alpha = 0.5) +
  ggtitle("Single dosing") + 
  theme_bw() +
  ytitle +
  xtitle +
