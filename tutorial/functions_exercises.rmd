...
Objective: Test the new xGx user's understanding of xgxr functions</p>
...
```{r plotting_question1, exercise=TRUE, check=TRUE}
# Setup 
gg <- ggplot(data = pk_data, aes(x = NOMTIME,
                                 y = LIDV,
                                 group = DOSE,
...
Can you add an Emax model fit to this data as well?

```{r plotting_question2, exercise=TRUE, check=TRUE}
# Setup
...
print(gg)
```
```{r plotting_question2-hint, check=TRUE}
# Use the plotting functions from the previous questions
# (xgx_geom_smooth_emax)
```

```{r plotting_question2-solution, check=TRUE}
# Setup 
# Setup 
gg <- ggplot(data = continuous_pk_data_wide, aes(x = NOMTIME,
                                 y = PD,
