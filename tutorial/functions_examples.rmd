...
for calculating and plotting default confidence intervals and 
also `xgx_geom_pi` for percentile intervals.
...
#### Binning non-discretized x axis with `xgx_stat_ci()`
```{r, fig.width=5, fig.height=3, warning=FALSE, error=FALSE, message=FALSE}
# Example of binned x values by quantile
gg <- xgx_plot(pkpd_data_wide,
         aes(x = CONC, response = ORDINAL_LEVELS, color = ORDINAL_LEVELS))
gg <- gg + xgx_stat_ci(bins = 6, distribution = "ordinal")
print(gg)
```
...
# Example of binned x values by quantile
gg <- xgx_plot(pkpd_data_wide,
         aes(x = CONC, response = ORDINAL_LEVELS))
gg <- gg + xgx_stat_ci(breaks = c(0.1, 0.2, 0.5, 1, 3), distribution = "ordinal")
print(gg)
```
